...
> **生成日期**: 2026-06-16 · **最后更新**: 2026-07-25

---

## 这是什么

一个元层次方法论项目——从 **22 个源项目**中系统提取可复用的方法论模式。源项目包括作者自建项目（7 个已公开、4 个未公开）以及对他人开源项目或论文的独立分析（11 个）。完整清单和分类见 `explorations/source_projects_summary.md`。
...
多轮正式审查（跨 GPT / Kimi / Qwen / GLM 四种后端），另有 1 次异后端复编码。累计 100 项独立发现（详见 `reviews/finding_register.md`；`explorations/finding_register.json` 为 GPT-5.6-Sol 重提取实验的早期产物，99→64 计数口径不同，非权威版本），审查记录见 `reviews/`。基于 Kohli (2026-05) 的独立性分析，有效独立视角约 2-3 个。关键数据点：
...
每张卡片记录一个源项目或外部压力测试的方法论特征提取结果，可在 `explorations/` 目录中查看。未公开源项目的详细证据卡不在本仓库中。
...
| **跨项目模式提取** | **未检索到同类工作** | **22 项目系统提取（搜索存在已知局限，见上文）** |
...
| 来源 | 验证了什么 | 修正了什么 |
|------|-----------|-----------|
| Kohli (2026-05) | 4-5 模型架构是最优配置 | 多轮审查 ≈ 2-3 个有效独立视角，不应声称"全部独立" |
...
2. **把散落的碎片拼成模式**——22 个项目的方法论文档分散在各处，提取框架提供了跨项目对比的流程
3. **让方法论被外部验证**——整理成结构化形式后，可以被对比、被质疑、被学术文献交叉验证

**核心局限**：框架在 22 个源项目上运行过——11 个外部分析 + 7 个自建公开 + 4 个自建未公开。其中 11 个有独立证据卡（`explorations/`），其余 11 个的分析过程记录在 `reviews/` 审查报告中。可提取程度与项目的方法论自觉正相关：主动记录AI协作过程的项目可提取大部分方法论组件；仅保留被动代码痕迹的项目可提取内容骤降至少数几个组件——框架无法从不存在的信息中提取信息。所有项目的提取和审查由同一操作者执行。完整清单和分类见 `explorations/source_projects_summary.md`。框架的有效性是否泛化到其他作者的工作习惯和不同操作者的提取方式，**尚未验证**。
