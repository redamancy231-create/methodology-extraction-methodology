...
**核心局限**：框架目前只在 19 个 AI 协作项目上运行过——其中 **16 个为作者自有项目**、**3 个为外部作者项目**（NPGS/baopinshui、ml-quant-trading/initial-d、CrossCheck/sburl）。自有项目的可提取率因项目类型差异很大（工具类 ~100% → 元项目类 ~81% → 部分未公开项目低至 50%），外部项目则骤降至 37-69%。所有项目的提取和审查由同一操作者执行。详细梯度见 `explorations/source_projects_summary.md`。框架的有效性是否泛化到其他作者的工作习惯和不同操作者的提取方式，**尚未验证**。
