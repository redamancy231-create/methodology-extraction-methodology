...
**核心侷限**：框架目前只在 19 個 AI 協作專案上執行過——其中 **16 個為作者自有專案**、**3 個為外部作者專案**（NPGS/baopinshui、ml-quant-trading/initial-d、CrossCheck/sburl）。自有專案的可提取率因專案型別差異很大（工具類 ~100% → 元專案類 ~81% → 部分未公開專案低至 50%），外部專案則驟降至 37-69%。所有專案的提取和審查由同一操作者執行。詳細梯度見 `explorations/source_projects_summary.md`。框架的有效性是否泛化到其他作者的工作習慣和不同操作者的提取方式，**尚未驗證**。
