...
一個元層次方法論專案——從 **19 個專案**（含自有 AI 協作專案、外部開源專案、學術壓力測試）中系統提取可複用的方法論模式。
...
專案自身的 G5 可追溯審計結論：**無一方法論模式滿足 ≥3 個源專案的穩定元件門檻。** 16 個自有專案來自同一作者、同一時期的 AI 協作實踐，3 個外部專案（NPGS/ml-quant-trading/CrossCheck）來自其他作者但由同一操作者提取——這個樣本量不足以支撐泛化的方法論提取。
...
2. **把散落的碎片拼成模式**——19 個專案的方法論文件分散在各處，提取框架提供了跨專案對比的流程
3. **讓方法論被外部驗證**——整理成結構化形式後，可以被對比、被質疑、被學術文獻交叉驗證

**核心侷限**：框架目前只在 19 個 AI 協作專案上執行過——其中 **16 個為作者自有專案**、**3 個為外部作者專案**（NPGS/baopinshui、ml-quant-trading/initial-d、CrossCheck/sburl）。可提取程度與專案的方法論自覺正相關：主動記錄AI協作過程的專案可提取大部分方法論組件；僅保留被動程式碼痕跡的專案可提取內容驟降至少數幾個組件——框架無法從不存在的資訊中提取資訊。所有專案的提取和審查由同一操作者執行。按專案類型的定性梯度見 `explorations/source_projects_summary.md`。框架的有效性是否泛化到其他作者的工作習慣和不同操作者的提取方式，**尚未驗證**。
