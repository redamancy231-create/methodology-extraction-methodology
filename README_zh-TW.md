...
一個元層次方法論專案——從 **20 個專案**（含自有 AI 協作專案、外部開源專案、學術壓力測試）中系統提取可複用的方法論模式。
...
專案自身的 G5 可追溯審計結論：**無一方法論模式滿足 ≥3 個源專案的穩定元件門檻。** 源專案中，部分為作者自建專案，部分為對他人開源專案的獨立分析（如 NPGS/ml-quant-trading/CrossCheck）。所有專案的提取和審查由同一操作者執行——這個樣本量不足以支撐泛化的方法論提取。
...
10 輪正式審查（來自 4 個審查後端：GPT-5.5/Kimi-K2.7/Qwen3.7-Max/GLM-5），另有 1 次異後端復編碼（Phase 3.5）。累計 58 項發現，全部閉合。基於 Kohli (2026-05) 的獨立性分析，有效獨立視角約 2-3 個。關鍵資料點：
...
每張卡片記錄一個源專案或外部壓力測試的方法論特徵提取結果。另有 10 張未公開專案的證據卡保留在本地（詳見 `explorations/source_projects_summary.md` 口徑說明）。
...
| **跨專案模式提取** | **未檢索到同類工作** | **12 專案系統提取（搜尋存在已知侷限，見上文）** |
...
2. **把散落的碎片拼成模式**——20 個專案的方法論文件分散在各處，提取框架提供了跨專案對比的流程
3. **讓方法論被外部驗證**——整理成結構化形式後，可以被對比、被質疑、被學術文獻交叉驗證

**核心侷限**：框架目前只在 20 個 AI 協作專案上執行過——其中部分為作者自建專案、部分為對他人專案的獨立分析（如 NPGS/ml-quant-trading/CrossCheck）。可提取程度與專案的方法論自覺正相關：主動記錄AI協作過程的專案可提取大部分方法論組件；僅保留被動程式碼痕跡的專案可提取內容驟降至少數幾個組件——框架無法從不存在的資訊中提取資訊。所有專案的提取和審查由同一操作者執行。按來源和類型的詳細分類見 `explorations/source_projects_summary.md`。

**證據卡覆蓋**：上述 20 個源專案中，本倉庫僅包含 11 張公開證據卡（8 張自建專案 + 3 張外部分析專案）。其餘專案的證據卡因涉及未公開專案而保留在本地——聲稱的專案總數不等於公開可驗證的證據卡數。框架的有效性是否泛化到其他作者的工作習慣和不同操作者的提取方式，**尚未驗證**。
