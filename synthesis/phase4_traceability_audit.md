...
- 量化策略项目 A：核心教训是 Phase 0 必须锁可证伪死亡判据，项目用 18 个版本买来“评审擅长这一步做对了吗，不擅长这件事该不该做”的教训。证据：`量化策略项目 A CLAUDE.md:174`。
...
- 对照实验项目 D：有预登记和冻结机制，属于“premise gate 正例设计”，不是“缺失 premise gate 导致错误延迟”的直接证据。证据：`对照实验项目 D 预登记_pilot.md:63`。
- 量化竞赛项目 C：有 IC 快速验证和活跃边界提醒，但项目未闭合，且证据更多指向“迁移验证”而非 premise-review scope。证据：`量化竞赛项目 C CLAUDE.md:35`。
...
- 量化策略项目 A：Kimi+GLM 终期双审提出重启门槛与数据窥探等修订建议。证据：`量化策略项目 A CLAUDE.md:26`，`量化策略项目 A 17\Kimi审阅结论_终期总结报告.md:11`。
- 方法论评估项目 B：四轮独立审查，Codex/Qwen/Kimi/Opus 各自覆盖工程bug、数据一致性和方法有效性。证据：`方法论评估项目 B CLAUDE.md:9`，`方法论评估项目 B archive\cross-round-findings-matrix.md:60`。
...
- 量化策略项目 A：项目在投资上失败，但产出负迁移检验、事前否决位和幂等性验证等可迁移资产。证据：`量化策略项目 A CLAUDE.md:172`，`:174`，`:177`。
...
- 方法论评估项目 B：四轮独立审查覆盖不同类型缺陷。证据：`方法论评估项目 B CLAUDE.md:9`。
...
- 量化策略项目 A：Kimi/GLM终期双审提出重要修订。证据：`量化策略项目 A CLAUDE.md:26`。
