...
|------|------|:--:|
| 量化策略项目 A | 量化（多头轮动策略） | ✅ |
| 方法论评估项目 B | 方法论（LLM 报告完整性测试——分析他人论文但项目本身未公开） | ❌ |
| 量化竞赛项目 C | 量化竞赛 | ❌ |
| 对照实验项目 D | 方法论/对照实验 | ❌ |
...
一个值得记录的观察：父框架（AI 协作框架）有整个语料中最强的 CLAUDE.md（95 行，LSP 优先策略、审查独立性硬约束、Provenance 级别），但从它提取出的两个衍生工具——**Independent Review Toolkit** 和 **Prompt-TDD Methodology**——完全没有 CLAUDE.md。GPT-5.6-Sol 对两个项目的 C5（AI 协作方法论文档）均判定为 ❌。

这不是疏忽——SOP 和案例复盘在各自项目中承担了部分操作手册功能。但它揭示了"方法论→可执行工具"的提取过程中的一个衰减模式：**父框架的 Agent 治理基础设施在衍生项目中不自动继承**。提取的是方法论内容（SOP、实验协议），不是方法论基础设施（CLAUDE.md 的编写规范、Agent 边界定义、审查独立性约束）。
...
**审计结论（G5）**：按初始严格门槛（≥3 个独立作者源项目 + 反例测试通过），P1-P8 无一定标。表中 P2、P1、P4 达到了 ≥3 计数的字面门槛，但全部来自同一作者——无法区分"通用方法论"和"个人工作习惯"。"对失败诚实"（未列入 P1-P8 目录）在扩展集中达到 ≥3 源门槛：量化策略项目 A（标注投资失败）、Prompt-TDD Methodology（阴性实验公开）。该观察不改变其余模式的试行状态。

> 注：当前项目（方法论提取方法论）的README同样标注了"不成熟"，但当前项目不在22个源项目清单中，不计入G5支持统计。
